...
Shader "Snow Family/Specular/Rim Light Alhpa - Reflection Alhpa"
{
        Properties {
        ...
        CGPROGRAM
          #pragma surface surf WrapLambert approxview alpha exclude_path:prepass exclude_path:deferred noforwardadd
		  //#pragma debug
        ...
        //c.rgb = s.Albedo * _LightColor0.rgb * (diff * atten) + (_LightColor0.rgb * specCol * _SpecPower);
        	...
        	c.rgb = 
			(s.Albedo * _LightColor0.rgb ) * (diff)		//디퓨즈+라이트
			+ ((((_LightColor0.rgb) * (_RimColor * 1) * (s.Albedo * 2))) * _RimPower * pow(rimLight1, _RimRange))		//림라이트
			+ (_LightColor0.rgb * specCol * _SpecPower ) 		//스펙큘러
			;
        ...
        void surf (Input IN, inout MySurfaceOutput o)
		{
			...
			fixed rim = 0.1f + saturate( dot(normalize(IN.viewDir), o.Normal) );
			...
			*/
			...
			o.Alpha = _ColorDamage.a;	//c.a;
			//o.GlossColor = s.rgb;
			o.Gloss = s.rgb  ; //스펙큘러 마스크맵의 R채널을 마스크로 사용
			//o.Specular = _Shininess;


			o.Emission = ((s.a * ref.rgb * _ReflectionPower) * _LightColor0.rgb) * pow(rim, _RimPower) ; // 리플렉션 효과
