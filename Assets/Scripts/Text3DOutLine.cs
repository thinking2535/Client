...
public class Text3DOutLine : MonoBehaviour
{
    ...
    public Int32 doubleResolution = 1024;

    private TextMesh textMesh;
    private MeshRenderer meshRenderer;
    private bool isDead = false;
    public Camera NowCamera = null;
    ...
    void LateUpdate()
    {
        if (isDead && NowCamera) return;
        //현재 원본 Text의 월드 좌표를 스크린 포인트로 맵핑합니다.
        Vector3 screenPoint = NowCamera.WorldToScreenPoint(transform.position);
        ...
        for (Int32 i = 0; i < transform.childCount; i++)
        {
            ...
            Vector3 worldPoint = NowCamera.ScreenToWorldPoint(screenPoint + pixelOffset);
