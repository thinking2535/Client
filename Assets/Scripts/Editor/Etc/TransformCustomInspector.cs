...
public class TransformCustomInspector : Editor
{
	...
	void OnEnable()
	{
		...
		mRotHint = serializedObject.FindProperty("m_LocalEulerAnglesHint");
	...
	Axes CheckDifference(SerializedProperty property)
	{
		...
		if (property.hasMultipleDifferentValues)
		{
			...
			{
				Object obj = serializedObject.targetObjects[i];
	...
	void DrawRotation()
	{
		GUILayout.BeginHorizontal();
		{
			...
			Vector3 prevValue = mRotHint.vector3Value;

			EditorGUILayout.PropertyField(mRotHint.FindPropertyRelative("x"));
			EditorGUILayout.PropertyField(mRotHint.FindPropertyRelative("y"));
			EditorGUILayout.PropertyField(mRotHint.FindPropertyRelative("z"));

			if (reset)
			{
				mRot.quaternionValue = Quaternion.identity;
				mRotHint.vector3Value = Vector3.zero;
			}
			else if (prevValue != mRotHint.vector3Value)
			{
				mRot.quaternionValue = Quaternion.Euler(mRotHint.vector3Value);
			}
			/*
